:SCEP: 2
:Title: Sample reStructured Text SCEP Template
...
This SCEP provides a boilerplate or sample template for creating your
own reStructuredText SCEPs.  In conjunction with the content guidelines
in SCEP 1 [#SCEP-1]_, this should make it easy for you to conform your own
SCEPs to the format outlined below.

Note: if you are reading this SCEP via the web, you should first grab
the text (reStructuredText) source of this SCEP in order to complete
the steps below.  **DO NOT USE THE HTML FILE AS YOUR TEMPLATE!**

To get the source this (or any) SCEP, look at the top of the HTML page
and click on the "Source" link.

Rationale
=========

SCEP submissions come in a wide variety of forms, not all adhering
...
SCEP submission won't get automatically rejected because of form.

ReStructuredText is offered as an alternative to plaintext SCEPs, to
allow SCEP authors more functionality and expressivity, while
...
To use this template, you must first decide whether your SCEP is going
to be an Informational or Standards Track SCEP.  Most SCEPs are
Standards Track because they propose a new feature for the Structured Commons
infrastructure and network protocols.  When in doubt,
read SCEP 1 for details or contact the SCEP editors
<editors@structured-commons.org>.

Once you've decided which type of SCEP yours is going to be, follow the
directions below.

- Make a copy of this file (``.rst`` file, **not** HTML!)  and perform
  the following edits.

- Replace the "SCEP: 2" header with "SCEP: XXX" since you don't yet have
  a SCEP number assignment.

- Change the Title header to the title of your SCEP.
...
- If there is a mailing list for discussion of your new feature, add a
  ...
  sc-discuss@structured-commons.org, or if discussions should be sent to
  you directly.  Most Informational SCEPs don't have a Discussions-To
  header.

- Change the Status header to "Draft".

- For Standards Track SCEPs, change the Type header to "Standards
  Track".

- For Informational SCEPs, change the Type header to "Informational".

- For Standards Track SCEPs, if your feature depends on the acceptance
  of some other currently in-development SCEP, add a Requires header
  right after the Type header.  The value should be the SCEP number of
  the SCEP yours depends on.  Don't add this header if your dependent
  feature is described in a Final SCEP.
...
- Leave Post-History alone for now; you'll add dates to this header
  each time you post your SCEP to sc-discuss@structured-commons.org.  If
  you posted your SCEP to the lists on August 14, 2001 and September 3,
  2001, the Post-History header would look like::

      Post-History: 2001-08-14, 2001-09-03

  You must manually add new dates and check them in.  If you don't
  have check-in privileges, send your changes to the SCEP editors.

- Add a Replaces header if your SCEP obsoletes an earlier SCEP.  The
  value of this header is the number of the SCEP that your new SCEP is
  replacing.  Only add this header if the older SCEP is in "final"
  form, i.e. is either Accepted, Final, or Rejected.  You aren't
  replacing an older open SCEP if you're submitting a competing idea.

- Now write your Abstract, Rationale, and other content for your SCEP,
...
- Update your References and Copyright section.  Usually you'll place
  your SCEP into the public domain, in which case just leave the
...
- Send your SCEP submission to the SCEP editors at
  editors@structured-commons.org.



ReStructuredText SCEP Formatting Requirements
=============================================

The following is a SCEP-specific summary of reStructuredText syntax.
...
SCEP headings must begin in column zero and the initial letter of each
...
If there are more than three levels of sections in your SCEP, you may
...
You shouldn't have more than five levels of sections in your SCEP.  If
...
When referencing an external web page in the body of a SCEP, you should
...
is a single word.  For example::
    ...
    In this paragraph, we refer to the `Structured Commons web site`_.

An explicit target provides the URL.  Put targets in a References
section at the end of the SCEP, or immediately after the reference.
Hyperlink targets begin with two periods and a space (the "explicit
markup start"), followed by a leading underscore, the reference text,
a colon, and the URL (absolute or relative)::

    .. _Structured Commons web site: http://www.structured-commons.org/
...
automatically at the end of the References section of the SCEP, along
...
When referring to another SCEP, include the SCEP number in the body
text, such as "SCEP 1".  The title may optionally appear.  Add a
footnote reference following the title.  For example::

    Refer to SCEP 1 [2]_ for more information.

Add a footnote that includes the SCEP's title.  It may
...
followed by the footnote body.  For example::
    ...
    .. [2] SCEP 1, "The Structured Commons Enhancement Proposal Process"
       (http://www.structured-commons.org/scep0001.html)

If you decide to provide an explicit URL for a SCEP, please use this as
the URL template::

    http://www.structured-commons.org/scepXXXX.html

SCEP numbers in URLs must be padded with zeros from the left, so as to
be exactly 4 characters wide, however SCEP numbers in the text are
never padded.

During the course of developing your SCEP, you may have to add, remove,
...
other characters are allowed).  For example::
    ...
    Refer to SCEP 1 [#SCEP-1]_ for more information.

    References
    ==========

    .. [#SCEP-1] SCEP 1, "The Structured Commons Enhancement Proposal Process"
       (http://www.structured-commons.org/scep0001.html)

Footnotes and footnote references will be numbered automatically, and
the numbers will always match.  Once a SCEP is finalized, auto-numbered
...
If your SCEP contains a diagram, you may include it in the processed
...
Since this image will not be visible to readers of the SCEP in source
...
your SCEP in source form, please consider including a descriptions of
...
The processing of reStructuredText SCEPs is done using Docutils_.  If
...
.. _post a message:
   mailto:docutils-users@lists.sourceforge.net?subject=SCEPs
...
.. [#SCEP-1] SCEP0001. The Structured Commons Enhancement Proposal Process.
   (http://www.structured-commons.org/scep0001.html)
